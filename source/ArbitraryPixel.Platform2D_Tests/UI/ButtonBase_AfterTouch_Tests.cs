...
{
    [TestClass]
    public class ButtonBase_AfterTouch_Tests : ButtonBase_Tests
    {
        ...
        public void UpdateWhenReleasedOutOfBoundsShouldFireReleasedEventWithLastKnownLocation()
        {
            ...
            _subscriber.When(x => x(_sut, Arg.Any<ButtonEventArgs>())).Do(
                x =>
                {
                    ...
                    Assert.AreEqual<Vector2>(_sut.Bounds.Centre, args.Location);
        ...
        {
            _mockSurfaceInputManager.GetSurfaceState().Returns(new SurfaceState(_sut.Bounds.Centre, false));
